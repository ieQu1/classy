...
-module(classy_discovery_etcd_SUITE).
...
t_discover(_Config) ->
  ...
  {ok, ['n1@127.0.0.1']} = classy_discovery_strategy:discover(classy_discovery_etcd, ?OPTIONS).

t_lock(_Config) ->
  ok = meck:expect(ekka_httpc, put, fun(_Server, _Path, _Params, _Opts) ->
                                            {ok, jsone:decode(<<"{\"errorCode\": 0}">>)}
                                    end),
  ok = classy_discovery_strategy:lock(classy_discovery_etcd, ?OPTIONS).

t_unlock(_) ->
  ok = meck:expect(ekka_httpc, delete, fun(_Server, _Path, _Params, _Opts) ->
                                               {ok, jsone:decode(<<"{\"errorCode\": 0}">>)}
                                       end),
  ok = classy_discovery_strategy:unlock(classy_discovery_etcd, ?OPTIONS).

t_register(_) ->
  ok = meck:new(classy_discovery_sup, [non_strict, passthrough, no_history]),
  ok = meck:expect(classy_discovery_sup, start_child, fun(_, _) -> {ok, self()} end),
  ok = meck:expect(ekka_httpc, put, fun(_Server, _Path, _Params, _Opts) ->
                                            {ok, jsone:decode(<<"{\"errorCode\": 0}">>)}
                                    end),
  ok = classy_discovery_strategy:register(classy_discovery_etcd, ?OPTIONS),
  ok = meck:unload(classy_discovery_sup).

t_unregister(_) ->
  ok = meck:expect(ekka_httpc, delete, fun(_Server, _Path, _Params, _Opts) ->
                                               {ok, jsone:decode(<<"{\"errorCode\": 0}">>)}
                                       end),
  ok = meck:expect(classy_discovery_sup, stop_child, fun(_) -> ok end),
  ok = classy_discovery_strategy:unregister(classy_discovery_etcd, ?OPTIONS),
  ok = meck:unload(classy_discovery_sup).
...
t_etcd_set_node_key(_) ->
  ...
  {ok, #{<<"errorCode">> := 0}} = classy_discovery_etcd:etcd_set_node_key(?OPTIONS).
...
t_restart_process(_) ->
  ...
  try
    ok = ekka_ct:wait_running2(Node),
    Pid = erpc:call(Node, erlang, whereis, [classy_discovery_etcd]),
    ...
    ekka_ct:wait_running(Node, 2_000)
  after
